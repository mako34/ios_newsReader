...
CGFloat ProgressHUDRingRadius = 14;
CGFloat ProgressHUDRingThickness = 6;
...
+ (void)show {
    [[ProgressHUD sharedView] showProgress:-1 status:nil maskType:ProgressHUDMaskTypeNone];
}

+ (void)showWithStatus:(NSString *)status {
    [[ProgressHUD sharedView] showProgress:-1 status:status maskType:ProgressHUDMaskTypeNone];
}

+ (void)showWithMaskType:(ProgressHUDMaskType)maskType {
    [[ProgressHUD sharedView] showProgress:-1 status:nil maskType:maskType];
}

+ (void)showWithStatus:(NSString*)status maskType:(ProgressHUDMaskType)maskType {
    [[ProgressHUD sharedView] showProgress:-1 status:status maskType:maskType];
}

+ (void)showProgress:(CGFloat)progress {
    [[ProgressHUD sharedView] showProgress:progress status:nil maskType:ProgressHUDMaskTypeNone];
}

+ (void)showProgress:(CGFloat)progress status:(NSString *)status {
    [[ProgressHUD sharedView] showProgress:progress status:status maskType:ProgressHUDMaskTypeNone];
}

+ (void)showProgress:(CGFloat)progress status:(NSString *)status maskType:(ProgressHUDMaskType)maskType {
    [[ProgressHUD sharedView] showProgress:progress status:status maskType:maskType];
}

#pragma mark - Show then dismiss methods

+ (void)showSuccessWithStatus:(NSString *)string {
    [ProgressHUD showImage:[UIImage imageNamed:@"OMUIKit.bundle/OMProgressHUD-success.png"] status:string];
}

+ (void)showErrorWithStatus:(NSString *)string {
    [ProgressHUD showImage:[UIImage imageNamed:@"OMUIKit.bundle/OMProgressHUD-error.png"] status:string];
}

+ (void)showImage:(UIImage *)image status:(NSString *)string {
    NSTimeInterval displayInterval = [[ProgressHUD sharedView] displayDurationForString:string];
    [[ProgressHUD sharedView] showImage:image status:string duration:displayInterval];
}
...
+ (void)popActivity {
    [ProgressHUD sharedView].activityCount--;
    if([ProgressHUD sharedView].activityCount == 0)
...
- (CAShapeLayer *)ringLayer {
    if(!_ringLayer) {
        ...
        _ringLayer = [self createRingLayerWithCenter:center radius:ProgressHUDRingRadius lineWidth:ProgressHUDRingThickness color:[UIColor whiteColor]];
...
- (CAShapeLayer *)backgroundRingLayer {
    if(!_backgroundRingLayer) {
        CGPoint center = CGPointMake(CGRectGetWidth(hudView.frame)/2, CGRectGetHeight(hudView.frame)/2);
        _backgroundRingLayer = [self createRingLayerWithCenter:center radius:ProgressHUDRingRadius lineWidth:ProgressHUDRingThickness color:[UIColor darkGrayColor]];
